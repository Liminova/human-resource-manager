...
class PerformanceGui(ctk.CTk):
    def __init__(self, master = None):
        ...
        def view_sales_performance(self):
            ...
            self.button = ctk.CTkButton(master=self.right_frame, text="View", font=("Century Gothic", 20, "bold"), command=(lambda: view_sales_performance(self)))
            self.button.configure(width=100, height = 40, font=("Century Gothic", 15, "bold"), corner_radius=10, fg_color="purple")
            self.button.place(relx=0.5, rely=0.9, anchor=tkinter.CENTER)
        ...
        self.button1 = ctk.CTkButton(master=self.left_frame, text="Add Sale", command=(lambda: [destroy_all_frames(self), add_sale(self)]))
        button_size(self.button1)
        self.button1.place(relx=0.5, rely=0.15, anchor=tkinter.CENTER)

        self.button2 = ctk.CTkButton(master=self.left_frame, text="View Sales Performance", command=(lambda: [destroy_all_frames(self), view_sales_performance(self)]))
        button_size(self.button2)
        self.button2.place(relx=0.5, rely=0.25, anchor=tkinter.CENTER)

        self.button3 = ctk.CTkButton(master=self.left_frame, text="Remove Sale", command=(lambda: [destroy_all_frames(self), remove_sale(self)]))
        button_size(self.button3)
        self.button3.place(relx=0.5, rely=0.35, anchor=tkinter.CENTER)

        self.button4 = ctk.CTkButton(master=self.left_frame, text="Get Sale Info", command=(lambda: [destroy_all_frames(self), get_sale_info(self)]))
        button_size(self.button4)
        self.button4.place(relx=0.5, rely=0.45, anchor=tkinter.CENTER)

        self.button5 = ctk.CTkButton(master=self.left_frame, text="Find Sale By", command=(lambda: [destroy_all_frames(self), find_sale_by(self)]))
