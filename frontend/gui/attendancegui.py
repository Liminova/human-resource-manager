...
class AttendanceGui(ctk.CTk):
    def __init__(self, master = None):
        ...
        def update_attendance(self):
            ...
            self.label0 = ctk.CTkLabel(master=self.right_frame, text="(You are currently update the attendance of: )", font=("Century Gothic", 14, "italic"))
        ...
        self.button1 = ctk.CTkButton(master=self.left_frame, text="Attendance Check", command=(lambda: [destroy_all_frames(self), attendance_check(self)]))
        button_size(self.button1)
        self.button1.place(relx=0.5, rely=0.15, anchor=tkinter.CENTER)

        self.button2 = ctk.CTkButton(master=self.left_frame, text="Update Attendace Check", command=(lambda : [destroy_all_frames(self), update_attendance(self)]))
        button_size(self.button2)
        self.button2.place(relx=0.5, rely=0.3, anchor=tkinter.CENTER)

        self.button3 = ctk.CTkButton(master=self.left_frame, text="Attendance Report", command=(lambda : [destroy_all_frames(self), get_report(self)]))
