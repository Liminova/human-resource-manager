...
class MenuDepartment:
    ...
    def admin(self) -> Result[None, str]:
        ...
        while True:
            ...
            match choice:
                ...
                case 4: last_msg: str = self.__add_employee()
                case 5: last_msg: str = self.__remove_employee()
                case 6: last_msg: str = self.__view()
                case 7: last_msg: str = self.__view_all()
                case 8: last_msg: str = self.__view_employees_not_belong_to_any_department()
    ...
    def __add_employee(self) -> str:
        ...
        empls[employee_selected_index].department_id = depts[dept_selected_index].dept_id
        depts[dept_selected_index].members.append(empls[employee_selected_index])
        if os.getenv("HRMGR_DB") == "TRUE":
            employee_repo.update_one(
                { "_id": empls[employee_selected_index].id },
                { "$set": empls[employee_selected_index].dict(include={"department_id"}) },
                upsert=True,
            )
            department_repo.update_one(
    ...
    def __remove_employee(self) -> str:
        ...
        employee_items = [f"{employee.name} ({employee.employee_id})" for employee in empls if employee.department_id == depts[dept_selected_index].dept_id]
    ...
    def __view(self) -> str:
        ...
        if not self.__logged_in_employee.is_admin:
            dept = [dept for dept in depts if dept.dept_id == self.__logged_in_employee.department_id]
            if len(dept) == 0:
                return NO_DEPARTMENT_MSG
            print(dept[0])
        else:
